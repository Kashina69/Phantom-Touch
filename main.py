...
try:
    while True:
        ...
        if result.multi_hand_landmarks:
            ...
            index_tip = (lm_px[8].x, lm_px[8].y)
            # Mode toggle detection (open hand + strong 3D motion intensity)
            now = time.time()
            ...
            if use_this_frame:
                if hand_conf >= HAND_CONFIDENCE_MIN and is_open_hand(lm_norm) and motion_intensity >= TOGGLE_INTENSITY_MIN:
            ...
            # Active area check for clutch (drawing mode)
